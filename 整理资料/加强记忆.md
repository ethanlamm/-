...
- 由于再次进入不会重新挂载`mounted`，若要求缓存组件并且每次进入都重新请求数据，那么请求数据的方法可写在`actiated`生命周期中
...
5.手写Promise(只实现基本功能，then 的链式调用和值穿透特性未实现)
...
```js
在vue2中，数据劫持是通过Object.defineProperty，这个 API 有一些缺陷，并不能检测对象属性的添加和删除，但vue2提供了set和delete方法
