...
Web API IntersectionObserver，提供了异步观察目标元素于视口交叉状态的方法
...
实现：网页通过添加一个`<script>元素`，向服务器请求 JSON 数据，服务器收到请求后，将数据放在一个指定名字的回调函数的参数位置传回来
...
**2.CORS**：服务器端设置Access-Control-Allow-Origin
...
![preview](https://segmentfault.com/img/bVbd8Ro/view)
...
- 【调试难度】 mobx>redux
  ...
  - redux：状态不可变，使用纯函数返回一个新的状态；,redux 提供时间回溯的开发工具，同时纯函数以及更少的抽象，让调试变得更加容易
  - mobx：状态可变，可对其直接修改，mobx中有更多的抽象和封装，调试比较困难，同时结果难预测
